...
const suggestionsByFile = changedFiles.map(async ({ filename, raw_url }) => {
  ...
  const input = `Create a list of tags and categories for a \`${iconName}\` icon. 
  
  For tags: Don't include words like: 'icon' and preferably use single words.
  
  For categories: Select from the following list:
  - arrows
  - shapes
  - weather
  - ui
  ...
  const response = await client.responses.create({
      ...
      input,
      text: {
        format: zodTextFormat(tagsSchema, "metadata"),
      },
  });

  const { tags: suggestedTags, categories: suggestedCategories }: IconMetadata = JSON.parse(response.output_text);

  console.log(`Suggesting tags for ${iconName}:`, suggestedTags);
  console.log(`Suggesting categories for ${iconName}:`, suggestedCategories);
