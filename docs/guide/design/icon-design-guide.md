...
To ensure accurate AI retrieval and search functionality, please follow these guidelines for metadata:

### Categories

Each icon must belong to at least one of the following standardized categories:

*   `arrows`: Directional indicators, carets, chevrons.
*   `shapes`: Geometric shapes, circles, squares, polygons.
*   `weather`: Sun, cloud, rain, temperature.
*   `ui`: User Interface elements, menus, toggles, controls.
*   `actions`: Verbs and actions like edit, save, delete, play, pause.
*   `layout`: Grids, lists, sorting, alignment, views.
*   `communication`: Mail, message, chat, phone.
*   `media`: Image, video, music, camera.
*   `files`: Documents, folders, file types.
*   `devices`: Hardware, computers, phones, printers.
*   `commerce`: Shopping, money, cards, tags.
*   `brands`: Logos and brand marks.
*   `time`: Clocks, calendars, schedules, timers.
*   `location`: Maps, pins, navigation, globe.
*   `users`: Person, group, account, profile.
*   `security`: Lock, shield, key, warning.
*   `editor`: Text formatting, alignment, tools.
*   `development`: Code, terminal, git, bug.
*   `custom`: Specific to Tubiao or uncategorized (use sparingly).

### Tags

*   **Synonyms**: Include common alternative names (e.g., `trash` for `garbage`, `bin`).
*   **Visual Description**: Describe shapes or visual elements (e.g., `triangle`, `dotted`, `outline`).
*   **Concepts**: Abstract concepts associated with the icon (e.g., `warning`, `love`, `success`).
*   **Language**: Use English for all tags.
*   **AI Generation**: Tags generated by AI tools should be reviewed for relevance and accuracy.
